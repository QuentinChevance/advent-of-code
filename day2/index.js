...
const secondPart = (data) => {
    return data.reduce((prevRes, round) => {
        if (shouldLose(round)) {
            return prevRes + resultScore.lose + reasonScore[getKeyByValue(losesAgainst,round[0])]
        }
        if (shouldWin(round)) {
            return prevRes + resultScore.win + reasonScore[getKeyByValue(winsOver,round[0])]
        }
        return prevRes + resultScore.draw + reasonScore[getKeyByValue(drawsAgainst,round[0])]
    }, 0)
}
